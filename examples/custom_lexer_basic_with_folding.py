...
class LexerNim(PyQt5.Qsci.QsciLexerCustom):
    ...
    def styleText(self, start, end):
        ...
        for i, token in enumerate(tokens):
            ...
            # Folding
            if token[0] == "proc":
                # First get the previous fold level
                current_line, current_index = self.parent().lineIndexFromPosition(start + i)
                previous_line = current_line - 1
                previous_fold_level = self.parent().SendScintilla(
                    PyQt5.Qsci.QsciScintilla.SCI_GETFOLDLEVEL, line - 1
                )
                print(previous_fold_level)
...
# Initialize the folding style
#editor.setFolding(PyQt5.Qsci.QsciScintilla.BoxedTreeFoldStyle)
